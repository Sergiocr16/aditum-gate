...
def read_events(device_path):
    ...
    for event in dev.read_loop():
        ...
        # 2) Justo después del prefijo, exigir '='
        elif typed_len == prefix_len + 1:
            if current_text[prefix_len] != '=':
                print("\nFalta '=' tras ADITUMGATE. Bloqueado.")
                denied()
                discarding = True
                current_text = ''
                continue
...
def procesar_texto(texto):
    ...
    upper = texto.upper()
    must = f"{REQUIRED_PREFIX}="
    if not upper.startswith(must):
        print("Código QR inválido: no inicia con ADITUMGATE=. Acceso denegado.")
        denied()
        return

    # Extraer dato tras el '='
    _, aditumData = texto.split("=", 1)

    loading()
    if doorType == "exit":
        print("Procesando salida...")
        r = requests.get(f'https://app.aditumcr.com/api/aditum-gate-verifier-exit-secure/{aditumData}/{doorId}')
        print(f"Código de estado: {r.status_code}\nRespuesta: {r.text}")
    elif doorType == "entry":
        print("Procesando entrada...")
        r = requests.get(f'https://app.aditumcr.com/api/aditum-gate-verifier-entry-secure/{aditumData}/{doorId}')
        print(f"Código de estado: {r.status_code}\nRespuesta: {r.text}")
    else:
        print("No autorizado")
        denied()
